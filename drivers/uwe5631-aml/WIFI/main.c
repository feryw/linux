...
void sprdwl_netif_rx(struct sk_buff *skb, struct net_device *ndev)
{
	...
	/* Update per-station RX stats */
	if (vif->mode == SPRDWL_MODE_AP) {
		struct ethhdr *eh = (struct ethhdr *)skb->data;
		struct sprdwl_peer_entry *peer;
		
		peer = sprdwl_find_peer_entry_using_addr(vif, eh->h_source);
		if (peer) {
			peer->rx_bytes_accum += skb->len;
			if (peer->last_rx_sec != sec) {
				/* Calculate rate for the *previous* second interval */
				u32 interval = sec - peer->last_rx_sec;
				if (interval == 1) {
					peer->rx_rate = (peer->rx_bytes_accum * 8) >> 10;
				} else {
					peer->rx_rate = 0;
				}
				/* Reset accumulators for the new second */
				peer->rx_bytes_accum = 0;
				peer->last_rx_sec = sec;
			}
		}
	}
...
static netdev_tx_t sprdwl_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	/* Update per-station TX stats */
	if (vif->mode == SPRDWL_MODE_AP) {
		struct ethhdr *eh = (struct ethhdr *)skb->data;
		struct sprdwl_peer_entry *peer;
		
		peer = sprdwl_find_peer_entry_using_addr(vif, eh->h_dest);
		if (peer) {
			peer->tx_bytes_accum += skb->len;
			if (peer->last_tx_sec != sec) {
				u32 interval = sec - peer->last_tx_sec;
				if (interval == 1) {
					peer->tx_rate = (peer->tx_bytes_accum * 8) >> 10;
				} else {
					peer->tx_rate = 0;
				}
				peer->tx_bytes_accum = 0;
				peer->last_tx_sec = sec;
			}
		}
	}
...
#endif /* UWE5621_FTR */
	if (ret) {
		wl_ndev_log(L_DBG, ndev, "%s drop msg due to TX Err\n", __func__);
...
#else
#define WIFI_MAC_ADDR_PATH "/lib/firmware/unisoc/wifimac.txt"
#endif
