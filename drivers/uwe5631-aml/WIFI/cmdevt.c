...
{
	struct sprdwl_msg_buf *msg;
	struct sprdwl_cmd_get_station *p;
	u8 *r_buf = (u8 *)sta;
	u16 r_len = sizeof(struct sprdwl_cmd_get_station);
	int ret;

	msg = sprdwl_cmd_getbuf(priv, ETH_ALEN, vif_ctx_id, SPRDWL_HEAD_RSP,
				WIFI_CMD_GET_STATION);
	if (!msg)
		return -ENOMEM;
	
	/* Send MAC address to firmware so it knows which station to query */
	p = (struct sprdwl_cmd_get_station *)msg->data;
	if (mac) {
		memcpy(p->mac, mac, ETH_ALEN);
		wl_info("%s: querying station %pM\n", __func__, mac);
	} else {
		memset(p->mac, 0, ETH_ALEN);
		wl_info("%s: querying with NULL MAC\n", __func__);
	}
	
	ret = sprdwl_cmd_send_recv(priv, msg, CMD_WAIT_TIMEOUT, r_buf, &r_len);
	
	if (ret) {
		wl_err("%s: get_station failed, ret=%d, mac=%pM, r_len=%d\n",
		       __func__, ret, mac ? mac : (u8 *)"\x00\x00\x00\x00\x00\x00", r_len);
	} else {
		wl_info("%s: get_station success, signal=%d, txfailed=%d\n",
			__func__, sta->signal, sta->txfailed);
	}
...
unsigned short sprdwl_rx_rsp_process(struct sprdwl_priv *priv, u8 *msg)
{
	...
	if (!cmd->data && SPRDWL_GET_LE32(hdr->mstime) == cmd->mstime &&
		...
		if (unlikely(hdr->status != 0)) {
			/* Only log TX_DATA errors at debug level to reduce spam */
			if (hdr->cmd_id == WIFI_CMD_TX_DATA) {
...
void sprdwl_event_frame(struct sprdwl_vif *vif, u8 *data, u16 len, int flag)
{
	...
	case SPRDWL_FRAME_NORMAL:
		/* Update peer signal strength if available */
		if (buf_len >= 10 + ETH_ALEN) {
			/* Management frame header: Frame Control (2) + Duration (2) + DA (6) + SA (6) */
			u8 *sa = buf + 10;
			struct sprdwl_peer_entry *peer;

			/* Find peer and update signal */
			peer = sprdwl_find_peer_entry_using_addr(vif, sa);
			if (peer && frame->signal > -100 && frame->signal < 0) {
				peer->signal = frame->signal;
			}
		}
