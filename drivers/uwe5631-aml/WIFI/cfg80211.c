...
static struct ieee80211_channel sprdwl_5ghz_channels[] = {
	/* All flags set to 0 to bypass regulatory restrictions */
				       ...
				       struct station_info *sinfo)
{
	...
	/* Always fill network stack statistics */
	...
	/* For AP mode, firmware doesn't support per-client query,
	 * so we only return basic stats from network stack.
	 * For STA mode, query firmware for signal/bitrate info.
	 */
	// if (vif->wdev.iftype != NL80211_IFTYPE_AP) {
	{
		/* Get current station info from firmware - only for STA mode */
		ret = sprdwl_get_station(vif->priv, vif->ctx_id, mac, &sta);
		if (ret)
			goto out;
				       ...
				       struct station_info *sinfo)
{
	...
	// if (vif->wdev.iftype != NL80211_IFTYPE_AP) {
	{
		...
		/* Fill TX power from wiphy configuration */
		if (wiphy->chandef.chan) {
			sinfo->txrate.tx_power = wiphy->chandef.chan->max_power;
			sinfo->filled |= STATION_INFO_TX_POWER;
		}

		/* Fill noise from firmware data */
		if (sta.noise != 0) {
			sinfo->noise = sta.noise;
			sinfo->filled |= STATION_INFO_NOISE;
		}
					...
					struct station_info *sinfo)
{
	...
	wl_ndev_log(L_DBG, ndev, "%s idx %d\n", __func__, idx);

	for (i = 0; i < MAX_LUT_NUM; i++) {
		struct sprdwl_peer_entry *peer = &intf->peer_entry[i];

		if (peer->ctx_id != vif->ctx_id)
			continue;

		if (is_zero_ether_addr(peer->tx.da))
			continue;
		...
		memcpy(mac, peer->tx.da, ETH_ALEN);
		wl_ndev_log(L_DBG, ndev, "Found station %pM at idx %d (lut %d)\n",
			    mac, idx, i);

		/* Attempt to get station info.
		 * sprdwl_cfg80211_get_station handles basic stats filling.
		 * Note: For AP mode, it might skip FW query, but we still want
		 * to return 0 to indicate we found a station.
		 */
		sprdwl_cfg80211_get_station(wiphy, ndev, mac, sinfo);

		/* Cache valid signal strength */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 83)
		if ((sinfo->filled & BIT(NL80211_STA_INFO_SIGNAL)) &&
		    (sinfo->signal < 0 && sinfo->signal > -100)) {
			peer->signal = sinfo->signal;
		}

		if (peer->signal != 0) {
			sinfo->signal = peer->signal;
			sinfo->filled |= BIT(NL80211_STA_INFO_SIGNAL);
		} else if ((sinfo->filled & BIT(NL80211_STA_INFO_SIGNAL)) &&
			   sinfo->signal > -128) {
			/* No cache, use firmware value if not invalid */
		} else {
			sinfo->filled &= ~BIT(NL80211_STA_INFO_SIGNAL);
		}
#else
		...
		if (peer->signal != 0) {
			sinfo->signal = peer->signal;
			sinfo->filled |= STATION_INFO_SIGNAL;
		} else if ((sinfo->filled & STATION_INFO_SIGNAL) &&
			   sinfo->signal > -128) {
			/* No cache, use firmware value if not invalid */
		} else {
			sinfo->filled &= ~STATION_INFO_SIGNAL;
		...
		/* Use per-client throughput stats tracked by driver */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 83)
		if (peer->tx_rate > 0 || !(sinfo->filled & BIT(NL80211_STA_INFO_TX_BITRATE))) {
			sinfo->txrate.legacy = peer->tx_rate / 100;
			sinfo->filled |= BIT(NL80211_STA_INFO_TX_BITRATE);
		}
		if (peer->rx_rate > 0 || !(sinfo->filled & BIT(NL80211_STA_INFO_RX_BITRATE))) {
			sinfo->rxrate.legacy = peer->rx_rate / 100;
			sinfo->filled |= BIT(NL80211_STA_INFO_RX_BITRATE);
		}
#else
		if (peer->tx_rate > 0 || !(sinfo->filled & STATION_INFO_TX_BITRATE)) {
			sinfo->txrate.legacy = peer->tx_rate / 100;
			sinfo->filled |= STATION_INFO_TX_BITRATE;
		}
		if (peer->rx_rate > 0 || !(sinfo->filled & STATION_INFO_RX_BITRATE)) {
			sinfo->rxrate.legacy = peer->rx_rate / 100;
				       ...
				       struct sprdwl_if_ops *ops)
{
	...
	/* Disable regulatory enforcement to enable all 5GHz channels */
	wiphy->regulatory_flags |= REGULATORY_CUSTOM_REG;
