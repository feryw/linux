 ...
 */
static void __maybe_unused si5351_drive_strength(struct si5351_priv *priv, enum si5351_clock clk, enum si5351_drive drive)
{
	u8 reg_val;
...
static void m88rs6060_calc_PLS_gold_code(u8 * pNormalCode, u32 PLSGoldCode)
{
	...
	int iPLSCnt =
	    sizeof(PLS_List) / sizeof(struct PLS_Table_t);
	int iPLSListIndex;
	u8 iPLSCode[3];
...
//		goto err;
	...
	dev_dbg(&i2c->dev, "pls mode %u, code %u\n", pls_mode, pls_code);
	...
	if (tmp1 == 0x8f) {
		...
		tmp1 &= 0x1f;
		dev_dbg(&i2c->dev, "ISI cnt = %u \n", tmp1);
		for (j = 0; j < (int)tmp1; j++) {
			regmap_write(dev->regmap, 0xf2, j);
			regmap_read(dev->regmap, 0xf3, &tsid[j]);
			dev_dbg(&i2c->dev, "ISI = %u \n", tsid[j]);
		}
		for (j = 0; j < (int)tmp1; j++)
			if (tsid[j] == isi) {
				mis = true;
	...
	if(p_info.type==MtFeType_DvbS2){
		...
		if(dev->config.ts_mode==MtFeTsOutMode_Serial){
			...
			if(input_datarate != 0)
				...
				divid_ratio = 0xff;
			dev_dbg(&i2c->dev, "MClk_KHz = %u,divid_ratio = %u \n", Mclk_KHz, divid_ratio);
...
#define M88rs6060_SNR_ITERATIONS 10
	...
	if (dev->fe_status & FE_HAS_LOCK) {
		unsigned int post_bit_error, post_bit_count;
