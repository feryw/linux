			    ...
			    int total_len)
{
	...
	for (num = 0; num < value->num; num++) {
		...
		skb = mm_single_buffer_unlink(mm_entry, pcie_addr);
		if (likely(skb)) {
			struct rx_mh_desc *mh_desc;
			
			/* For PCIE, rx_mh_desc is at the beginning of skb->data */
			mh_desc = (struct rx_mh_desc *)skb->data;
			csum = get_pcie_data_csum((void *)rx_if->intf, skb->data);
			...
			/* Extract RSSI/SNR for PCIE mode */
			if (msdu_desc->sta_lut_valid && msdu_desc->sta_lut_index < MAX_LUT_NUM) {
				rx_if->intf->signal[msdu_desc->sta_lut_index] = (s8)mh_desc->rss1;
				rx_if->intf->snr[msdu_desc->sta_lut_index] = mh_desc->snr1;
				rx_if->intf->data_rate[msdu_desc->sta_lut_index] = mh_desc->data_rate;
				rx_if->intf->rx_num[msdu_desc->sta_lut_index]++;
			}
				   ...
				   void *data, int len, int buffer_type)
{
	...
	if (unlikely(len < sizeof(struct rx_msdu_desc))) {
		...
		if (unlikely(!skb)) {
			...
			struct rx_msdu_desc *msdu_desc_temp;
			
			/* Extract RSSI/SNR from rx_mh_desc BEFORE skb_reserve */
			/* At this point, skb->data points to the beginning of the buffer */
			/* For SDIO: data layout is [hif_offset][rx_msdu_desc][payload] */
			/* rx_mh_desc is not present in SDIO mode, only in PCIE mode */
			msdu_desc_temp = (struct rx_msdu_desc *)(skb->data + mm_entry->hif_offset);
			
			/* Update RX count using sta_lut_index from descriptor */
		if (msdu_desc_temp->sta_lut_valid && msdu_desc_temp->sta_lut_index < MAX_LUT_NUM) {
			rx_if->intf->rx_num[msdu_desc_temp->sta_lut_index]++;
			/* For SDIO mode, rx_mh_desc with RSSI is not available */
			/* Set a default value to indicate link is active */
			if (rx_if->intf->signal[msdu_desc_temp->sta_lut_index] == 0) {
				rx_if->intf->signal[msdu_desc_temp->sta_lut_index] = -50;
			}
		}
