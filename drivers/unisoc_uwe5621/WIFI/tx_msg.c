...
void sprdwl_flush_mode_txlist(struct sprdwl_tx_msg *tx_msg, enum sprdwl_mode mode)
{
	...
	/* Reset last served peer index for this mode */
	for (i = 0; i < SPRDWL_AC_MAX; i++) {
		tx_msg->last_served_peer[mode][i] = 0;
	}
}
			     ...
			     int data_num)
{
	...
	if (priv->hw_type == SPRDWL_HW_PCIE)
		return data_num;
	/* Reduce max TX credit cap to 64 to prevent hardware bufferbloat.
	 * This forces packets to stay in the software queue where our
	 * Equal Share scheduler can manage them fairly.
	 */
	if (data_num > 64)
		data_num = 64;
				      ...
				      enum sprdwl_mode mode)
{
	...
	if (!list_empty(&tx_msg->xmit_msg_list.to_send_list)) {
		...
		if (credit <= 0) {
			/* HACK: Force TX if credits are stuck */
			credit = 1;
		}
...
unsigned int vo_ratio = 95;
unsigned int vi_ratio = 95;
unsigned int be_ratio = 95;
				   ...
				   enum sprdwl_mode mode)
{
	...
	if (total != 0 && send_num <= 0) {
		/* HACK: Force TX if credits are stuck
		 * If we have data but no credits, force sending 1 packet to unblock the queue.
		 * This prevents flow control deadlocks in multi-client scenarios.
		 */
		/* wl_warn_ratelimited("%s, %d: _fc_ no credit! Forcing 1 packet.\n",
		       __func__, __LINE__); */
		send_num = 1;
	}
	 ...
	 * FIXED: Improved fair distribution across all active peers
	 */
	for (i = 0; i < SPRDWL_AC_MAX; i++) {
		...
		/* Count total active peers for this AC */
		for (j = 0; j < MAX_LUT_NUM; j++) {
			if (p_list_num[i][j] > 0)
				active_peers++;
		}
		
		/* Calculate fair base share and remainder for equal distribution */
		if (active_peers > 0) {
			base_share = fp_num / active_peers;
			remainder = fp_num % active_peers;
		}
		
		/* Round Robin starting point - rotate for fairness */
		start_idx = (tx_msg->last_served_peer[mode][i] + 1) % MAX_LUT_NUM;

		/* First pass: Give each active peer equal base share */
		for (k = 0; k < MAX_LUT_NUM && fp_num > 0; k++) {
			j = (start_idx + k) % MAX_LUT_NUM;

			if (p_list_num[i][j] == 0)
				continue;
			
			/* Calculate packets to send for this peer */
			min_num = min(base_share, p_list_num[i][j]);
			
			/* Give one extra packet from remainder to first N peers */
			if (remainder > 0 && min_num > 0) {
				int extra = min(1, fp_num - min_num);
				min_num += extra;
				remainder--;
			}
			
			/* Ensure we don't exceed available packets or credits */
			min_num = min(min_num, fp_num);
			min_num = min(min_num, p_list_num[i][j]);
			
			wl_debug("TID=%d,PEER=%d, has=%d, base=%d, sending=%d, fp_remain=%d\n",
				 i, j, p_list_num[i][j], base_share, min_num, fp_num);
			
			if (min_num <= 0)
				continue;
			
			/* Update last served peer for next round */
			tx_msg->last_served_peer[mode][i] = j;

			q_list_num[i] -= min_num;
			fp_num -= min_num;
		...
		/* Second pass: Distribute any remaining credits fairly */
		/* This handles rounding errors and ensures all credits are used */
		while (fp_num > 0) {
			int distributed = 0;
			...
			for (k = 0; k < MAX_LUT_NUM && fp_num > 0; k++) {
				...
				if (atomic_read(&p_list->l_num) > 0) {
					int extra = 1; /* Distribute one packet at a time for fairness */
					extra = min(extra, atomic_read(&p_list->l_num));
					
					add_xmit_list_tail(tx_msg, p_list, extra);
					atomic_sub(extra, &p_list->l_num);
					atomic_sub(extra, &tx_list->mode_list_num);
					fp_num -= extra;
					distributed++;
					
					wl_debug("%s, %d, mode=%d, TID=%d, lut=%d, extra %d packet distributed (fp_num=%d)\n",
						 __func__, __LINE__, mode, i, j, extra, fp_num);
				}
			}
			
			/* Break if no packets were distributed in this round to avoid infinite loop */
			if (distributed == 0)
				break;
		}
	}
