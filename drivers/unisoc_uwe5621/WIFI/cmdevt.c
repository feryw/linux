...
int sprdwl_get_station(struct sprdwl_priv *priv, u8 vif_ctx_id,
		       const u8 *mac, struct sprdwl_cmd_get_station *sta)
{
	struct sprdwl_msg_buf *msg;
	struct sprdwl_cmd_get_station *cmd;
	u8 *r_buf = (u8 *)sta;
	u16 r_len = sizeof(struct sprdwl_cmd_get_station);
	int ret;

	msg = sprdwl_cmd_getbuf(priv, sizeof(*cmd), vif_ctx_id, SPRDWL_HEAD_RSP,
				WIFI_CMD_GET_STATION);
	if (!msg)
		return -ENOMEM;

	cmd = (struct sprdwl_cmd_get_station *)msg->data;
	memset(cmd, 0, sizeof(*cmd));
	if (mac)
...
int sprdwl_get_all_stations(struct sprdwl_priv *priv,
                            u8 vif_ctx_id,
                            struct sprdwl_station_info *sta_info,
                            int max_sta)
{
    struct sprdwl_intf *intf = (struct sprdwl_intf *)priv->hw_priv;
    struct sprdwl_peer_entry *peer;
    int i, count = 0;
	mutex_lock(&priv->peer_lock); /* Kunci mutex sebelum mengakses peer_entry */

    if (!intf || !sta_info) {
		mutex_unlock(&priv->peer_lock);
        return -EINVAL;
	}
    ...
    for (i = 0; i < MAX_LUT_NUM; i++) {
        ...
        peer = &intf->peer_entry[i];

        /*
         * FIX: Gunakan kondisi ini untuk memeriksa stasiun yang aktif.
         * Ini adalah cara yang aman untuk menghindari akses ke entri yang tidak valid
         * yang dapat menyebabkan crash.
         */
        if (peer->ctx_id == vif_ctx_id &&
            !is_zero_ether_addr(peer->tx.da) &&
            !is_multicast_ether_addr(peer->tx.da)) {

            ether_addr_copy(sta_info[count].mac_addr, peer->tx.da);

            /* Gunakan nilai sinyal yang sebenarnya dari RX descriptor */
            sta_info[count].signal = intf->signal[i];
            /* Jika SNR tersedia, kita bisa menambahkannya ke filled flags jika didukung */
            /* Untuk saat ini, kita hanya mengisi signal */
            sta_info[count].tx_packets = intf->tx_num[i];
            sta_info[count].rx_packets = intf->rx_num[i];
            sta_info[count].data_rate = intf->data_rate[i];

            count++;
        }
    }

	mutex_unlock(&priv->peer_lock); /* Lepaskan mutex setelah selesai */
...
void sprdwl_event_station(struct sprdwl_vif *vif, u8 *data, u16 len)
{
	...
	static u8 last_mac[ETH_ALEN] = {0};
	static u8 last_is_connect = 0xFF;
	static unsigned long last_jiffies = 0;
	unsigned long now = jiffies;
	unsigned long time_diff = 0;
	bool is_same_sta = false;

	/* Menambahkan log untuk debugging crash */
	wl_err("-> %s: Event %s untuk stasiun %pM (len=%u, ie_len=%u)\n",
	       __func__,
	       sta->is_connect ? "CONNECT" : "DISCONNECT",
	       sta->mac, len, le16_to_cpu(sta->ie_len));
	
	/* Dump raw data untuk debugging */
	wl_hex_dump(L_DBG, "event_station data: ", DUMP_PREFIX_OFFSET, 16, 1,
		    data, min_t(u16, len, 64), true);

	/* Cek apakah ini STA yang sama */
	is_same_sta = ether_addr_equal(last_mac, sta->mac);
	
	/* Hitung time difference */
	if (last_jiffies != 0) {
		time_diff = now - last_jiffies;
	}
	
	/* Filter 1: Ignore duplicate event (same MAC, same state, < 500ms) */
	if (is_same_sta && 
	    last_is_connect == sta->is_connect &&
	    time_diff < msecs_to_jiffies(500)) {
		wl_err("-> %s: Ignoring duplicate %s event for %pM (time_diff=%u ms)\n",
		       __func__,
		       sta->is_connect ? "CONNECT" : "DISCONNECT",
		       sta->mac, jiffies_to_msecs(time_diff));
		return;
	}

	/* Filter 2: Ignore rapid DISCONNECT after CONNECT (< 100ms) */
	/* Ini adalah bug firmware yang mengirim DISCONNECT langsung setelah CONNECT */
	if (is_same_sta && 
	    last_is_connect == 1 && sta->is_connect == 0 &&
	    time_diff < msecs_to_jiffies(100)) {
		wl_err("-> %s: Ignoring spurious DISCONNECT after CONNECT for %pM (time_diff=%u ms)\n",
		       __func__, sta->mac, jiffies_to_msecs(time_diff));
		/* Jangan update last_is_connect, biarkan tetap CONNECT */
		return;
	}

	/* Simpan state terakhir */
	memcpy(last_mac, sta->mac, ETH_ALEN);
	last_is_connect = sta->is_connect;
	last_jiffies = now;

	sprdwl_report_softap(vif, sta->is_connect,
			     sta->mac, sta->ie, le16_to_cpu(sta->ie_len));
}
...
unsigned short sprdwl_rx_event_process(struct sprdwl_priv *priv, u8 *msg)
{
	...
	/* Kunci mutex untuk event yang memodifikasi atau mengakses state stasiun */
	if (hdr->cmd_id == WIFI_EVENT_NEW_STATION ||
	    hdr->cmd_id == WIFI_EVENT_DISCONNECT ||
	    hdr->cmd_id == WIFI_EVENT_STA_LUT_INDEX) {
		mutex_lock(&priv->peer_lock);
	}
	...
	case WIFI_EVENT_DISCONNECT:
		wl_err("-> %s: Event DISCONNECT untuk vif %d\n", __func__, vif->ctx_id);
	...
	/* Buka kunci mutex setelah selesai menangani event */
	if (hdr->cmd_id == WIFI_EVENT_NEW_STATION ||
	    hdr->cmd_id == WIFI_EVENT_DISCONNECT ||
	    hdr->cmd_id == WIFI_EVENT_STA_LUT_INDEX) {
		mutex_unlock(&priv->peer_lock);
