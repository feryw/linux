...
static struct ieee80211_supported_band sprdwl_band_2ghz = {
	...
	.ht_cap.ampdu_factor = IEEE80211_HT_MAX_AMPDU_32K,
	.ht_cap.ampdu_density = IEEE80211_HT_MPDU_DENSITY_NONE,
	.ht_cap.mcs.rx_mask = {0xff, 0xff, 0, 0, 0, 0, 0, 0, 0, 0},
...
static struct ieee80211_supported_band sprdwl_band_5ghz = {
	...
	.vht_cap.vht_mcs.rx_highest = cpu_to_le16(867),
	.vht_cap.vht_mcs.tx_highest = cpu_to_le16(867),
};
				       ...
				       struct station_info *sinfo)
{
	...
	ret = sprdwl_get_station(vif->priv, vif->ctx_id, mac,
				 &sta);
	if (ret)
		goto out;
...
static int sprdwl_cfg80211_dump_station(struct wiphy *wiphy, struct net_device *ndev,
					int idx, u8 *mac, struct station_info *sinfo)
{
	...
	/* Hanya berjalan untuk antarmuka tipe AP atau GO */
	...
	/* Find the station at the requested index */
	for (i = 0; i < MAX_LUT_NUM; i++) {
		...
		if (peer->ctx_id == vif->ctx_id &&
			...
			if (count == idx) {
				/* Found the station at index */
				ether_addr_copy(mac, peer->tx.da);
				
				/* Unlock before calling sprdwl_get_station as it might sleep/wait */
				mutex_unlock(&priv->peer_lock);
				
				/* Use the command to get fresh info from firmware */
				/* This works for both SDIO and PCIE and provides more details */
				{
					struct sprdwl_cmd_get_station sta;
					struct sprdwl_rate_info *rate;
					
					memset(&sta, 0, sizeof(sta));
					ret = sprdwl_get_station(priv, vif->ctx_id, mac, &sta);
					if (ret == 0) {
						rate = (struct sprdwl_rate_info *)&sta;
						...
						sinfo->signal = sta.signal;
						/* Use internal counters for packets as they are updated by driver */
						/* Or use what firmware returns if it tracks it */
						sinfo->tx_packets = intf->tx_num[i];
						sinfo->rx_packets = intf->rx_num[i];
						
						/* Fill rate info from firmware response */
						if ((rate->flags & RATE_INFO_FLAGS_MCS) ||
							(rate->flags & RATE_INFO_FLAGS_VHT_MCS)) {
						...
						/* Also populate rxrate if possible, often same as txrate for display if not separate */
						sinfo->filled |= BIT(NL80211_STA_INFO_RX_BITRATE);
						sinfo->rxrate = sinfo->txrate;
					} else {
						/* Fallback to internal stats if command fails */
						sinfo->filled |= BIT(NL80211_STA_INFO_SIGNAL) |
								 BIT(NL80211_STA_INFO_TX_PACKETS) |
								 BIT(NL80211_STA_INFO_RX_PACKETS);
						sinfo->signal = intf->signal[i];
						sinfo->tx_packets = intf->tx_num[i];
						sinfo->rx_packets = intf->rx_num[i];
						if (intf->data_rate[i] > 0) {
							sinfo->filled |= BIT(NL80211_STA_INFO_RX_BITRATE);
							sinfo->rxrate.legacy = intf->data_rate[i] * 5;
						}
						ret = 0; 
					}
				}
		...
		struct sprdwl_priv *priv)
{
	...
	/* Force enable high performance flags */
	ht_info->cap |= (IEEE80211_HT_CAP_GRN_FLD | IEEE80211_HT_CAP_DSSSCCK40 | 
		...
		struct sprdwl_priv *priv)
{
	...
	/* Force enable high performance flags */
	vht_cap->cap |= (IEEE80211_VHT_CAP_SHORT_GI_80 | IEEE80211_VHT_CAP_RXLDPC | 
					 IEEE80211_VHT_CAP_TXSTBC | IEEE80211_VHT_CAP_MAX_MPDU_LENGTH_7991);
	memcpy(&vht_cap->vht_mcs, &sec2->vht_mcs_set,
			sizeof(struct ieee80211_vht_mcs_info));
			
	/* Optimize MCS map for 2 streams if restricted */
	if (vht_cap->vht_mcs.rx_mcs_map == 0xfff0) {
		vht_cap->vht_mcs.rx_mcs_map = 0xfffa;
		vht_cap->vht_mcs.tx_mcs_map = 0xfffa;
	}
}
